...
from debias.networks.resnet_cifar import ResNet18_base
...
def set_model():
    model = ResNet18_base().cuda()
    class_criterion = nn.CrossEntropyLoss()
    domain_criterion = nn.CrossEntropyLoss()
    
    class_network = nn.Linear(512, 10).cuda()
...
def validate(val_loader, model):
    ...
    return attrwise_acc_meter.get_unbiased_acc(), attrwise_acc_meter.get_acc_diff(), attrwise_acc_meter.get_bias_conflict(bc_classes)


def main():
    opt = parse_option()

    exp_name = f'adv-cifar10-{opt.exp_name}-lr{opt.lr}-bs{opt.bs}-seed{opt.seed}'
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        for key, val_loader in val_loaders.items():
            acc_unbiased, diff, bias_conflict = validate(val_loader, model)
