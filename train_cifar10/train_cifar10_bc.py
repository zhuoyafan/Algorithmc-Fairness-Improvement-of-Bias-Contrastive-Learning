...
def train(train_loader, cont_train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for images, labels, _, biases, _, _ in train_iter:
        ...
        labels, biases = labels.cuda().long(), biases.cuda().long()
...
def main():
    ...
    exp_name = f'bc-bb{opt.bb}-cifar10__corr{opt.corr}-{opt.exp_name}-lr{opt.lr}-bs{opt.bs}-cbs{opt.cbs}-w{opt.weight}-ratio{opt.ratio}-aug{opt.aug}-seed{opt.seed}'
