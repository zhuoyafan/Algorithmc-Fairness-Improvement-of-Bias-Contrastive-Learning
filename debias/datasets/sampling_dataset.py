...
class SamplingDataset:
    ...
    def set_dro_info(self):
        ...
        for i, t, b in zip(
            torch.arange((len(self.targets))), self.targets, self.bias_targets
        ):
            idx = t + (b * num_targets)
    ...
    def get_target_distro(self, target):
        ...
        target_distro = []
        for bias in range(num_biases):
            target_distro.append(
                torch.sum(
                    torch.logical_and(self.targets == target, self.bias_targets == bias)
                )
            )
    ...
    def get_kept_indices(self, target, target_prime, target_prime_new_distro):
        ...
        for bias, bias_distro in enumerate(target_prime_new_distro):
            tmp = torch.logical_and(
                self.targets == target_prime, self.bias_targets == bias
            )
        ...
        for target in range(num_targets):
            ...
            for target_prime in range(num_targets):
                ...
                if target_prime == target:
                    indices_target = list(
                        torch.arange(len(self.targets))[self.targets == target]
                    )
    ...
    def over_sample_ce(self):
        ...
        for group_idx in range(len(group_counts)):
            subsampled_indices = torch.arange(len(self.targets))[
    ...
    def get_targets_bin_distro(self, target_bin, num_targets, num_biases):
        ...
        for target in range(num_targets):
            target_distro = []
            for bias in range(num_biases):
                count = torch.logical_and(
                    self.targets == target, self.bias_targets == bias
                )
    ...
    def print_new_distro(self):
        ...
        logging.info("===================================")
        logging.info("Binary Labels Distribution: ")
        for target_idx in range(num_targets):

            logging.info(f"Binary Target {target_idx}")
            logging.info("---------------------------")
            target_distro = self.get_targets_bin_distro(
                target_idx, num_targets, num_biases
            )
            for target, distro in enumerate(target_distro):
                logging.info(f"Target {target}: {[x.item() for x in distro]}")

        logging.info("===================================")
        logging.info("Normal Label Distribution: ")

        for target in range(num_targets):
            target_distro = self.get_target_distro(target)
            logging.info(f"Target {target}: {[x.item() for x in target_distro]}")
