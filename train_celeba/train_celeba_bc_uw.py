...
from debias.utils.utils import (
    AverageMeter,
...
def parse_option():
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="batch_size of dataloader for contrastive loss",
    )
...
def set_model(train_loader, opt):
    ...
    criterion = BiasContrastiveLoss(
        confusion_matrix=train_loader.dataset.confusion_matrix, bb=opt.bb
    )
...
def train(train_loader, cont_train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for images, labels, _, biases, gc, _ in tqdm(train_iter, ascii=True):
        ...
        total_images, cont_labels, cont_biases = (
            total_images.cuda(),
            cont_labels.cuda(),
            cont_biases.cuda(),
        )
        ...
        ce_loss, con_loss = criterion(
            logits, labels, biases, cont_features, cont_labels, cont_biases, gc
        )
...
def validate(val_loader, model):
    ...
    with torch.no_grad():
        for _, (images, labels, _, biases, _, _) in enumerate(val_loader):
...
def main():
    ...
    train_loader = get_celeba(
        root, batch_size=opt.bs, target_attr=opt.task, split="train", aug=False
    )
    ...
    for epoch in range(1, opt.epochs + 1):
        logging.info(
            f"[{epoch} / {opt.epochs}] Learning rate: {scheduler.get_last_lr()[0]} weight: {opt.weight}"
        )
        ce_loss, con_loss, loss = train(
            train_loader, cont_train_loader, model, criterion, optimizer, epoch, opt
        )
        logging.info(
