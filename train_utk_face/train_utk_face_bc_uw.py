...
def train(train_loader, cont_train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for images, labels, _, biases, gc, _ in tqdm(train_iter, ascii=True):
        ...
        ce_loss, con_loss = criterion(logits, labels, biases, cont_features, cont_labels, cont_biases, gc)
...
def main():
    ...
    exp_name = f'bc-bb{opt.bb}-utk_face_{opt.task}-{opt.exp_name}-{opt.bias_rate}-lr{opt.lr}-bs{opt.bs}-cbs{opt.cbs}-w{opt.weight}-ratio{opt.ratio}-aug{opt.aug}-seed{opt.seed}'
