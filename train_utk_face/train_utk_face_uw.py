...
def train(train_loader, model, criterion, optimizer):
    ...
    for images, labels, _, biases, gc, _ in train_iter:
        bsz = labels.shape[0]
        labels, biases = labels.cuda().long(), biases.cuda()
        gc = gc.cuda()
...
def main():
    ...
    exp_name = f'uw-utk_face_{opt.task}-{opt.exp_name}-{opt.bias_rate}-lr{opt.lr}-bs{opt.bs}-seed{opt.seed}'
