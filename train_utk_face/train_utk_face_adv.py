...
def main():
    ...
    exp_name = f'adv-utk_face_{opt.task}-{opt.exp_name}-{opt.bias_rate}-lr{opt.lr}-bs{opt.bs}-seed{opt.seed}'
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        for tag in val_loaders.keys():
            if stats[f'{tag}/acc_unbiased'] > best_accs[tag]:
                ...
                # save_model(model[0], optimizers[0], opt, epoch, save_file)
            logging.info(
    ...
    # save_model(model[0], optimizers[0], opt, opt.epochs, save_file)
